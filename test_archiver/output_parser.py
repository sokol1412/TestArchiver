...
class RobotFrameworkOutputParser(XmlOutputParser):
    ...
    def startElement(self, name, attrs):
        if name in RobotFrameworkOutputParser.EXCLUDED_SECTIONS:
            ...
            self.archiver.begin_keyword(
                attrs.getValue("type"), "", attrs.getValue("type")
            )
            ...
            self.archiver.begin_log_message(
                attrs.getValue("level"), attrs.getValue("timestamp")
            )
            if self.archiver.config.log_level_ignored(attrs.getValue("level")):
                self.skipping_content = True
        elif name == "status":
            critical = (
                attrs.getValue("critical") == "yes"
                if "critical" in attrs.getNames()
                else None
            )
            self.archiver.begin_status(
                attrs.getValue("status"),
                attrs.getValue("starttime"),
    ...
    def startElement(self, name, attrs):
        if name in []:
            ...
            if not self.archiver.test_run_id:
                self.archiver.begin_test_run("xUnit parser", None, "xUnit", False, None)
            suite_name = (
                attrs.getValue("name")
                if "name" in attrs.getNames()
                else DEFAULT_SUITE_NAME
            )
            self.archiver.begin_suite(suite_name)
            errors = (
                int(attrs.getValue("errors")) if "errors" in attrs.getNames() else 0
            )
            failures = (
                int(attrs.getValue("failures")) if "failures" in attrs.getNames() else 0
            )
            suite_status = "PASS" if errors + failures == 0 else "FAIL"
            elapsed = (
                int(float(attrs.getValue("time")) * 1000)
                if "time" in attrs.getNames()
                else None
            )
            timestamp = (
                attrs.getValue("timestamp") if "timestamp" in attrs.getNames() else None
            )
            self.archiver.begin_status(
                suite_status, start_time=timestamp, elapsed=elapsed
            )
        elif name == "testcase":
...
class MochaJUnitOutputParser(XmlOutputParser):
    ...
    def startElement(self, name, attrs):
        if name == "testsuites":
            self.archiver.begin_test_run(
                "Mocha-JUnit parser", None, attrs.getValue("name"), False, None
            )
            ...
            while self.archiver.current_suite() and not suite_name.startswith(
                self.archiver.current_suite().full_name + "."
            ):
                self._end_suite()
        ...
        if name in []:
            ...
            if self.archiver.current_item_is_test():
                ...
                if (
                    ...
                    or current_test.name != test_name
                ):
                    self.archiver.end_test()
...
class MSTestOutputParser(XmlOutputParser):
    ...
    def startElement(self, name, attrs):
        if name in MSTestOutputParser.EXCLUDED_SECTIONS:
            ...
            self.archiver.begin_test_run(
                "MSTest parser", None, attrs.getValue("name"), False, None
            )
...
def parse_xml(xml_file, output_format, connection, config, build_number_cache):
    ...
    buffer_size = 65536
    test_archiver = archiver.Archiver(
        connection, config, build_number_cache=build_number_cache
    )
    ...
    if len(test_archiver.stack) != 1:
        raise Exception(
            "File parse error. Please check you used proper output format "
...

{
    ...
    """
    parser = configs.base_argument_parser(
        "Parse test automation output.xml files to SQL database."
    )
    parser.add_argument(
        "output_files",
        nargs="+",
    ...
    parser.add_argument(
        "--format",
        help="output format (default: robotframework)",
        default="robotframework",
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "The repository of the test cases. Used to differentiate between test with "
    ...
    parser.add_argument(
        ...
        action="append",
        help=(
            "Name of the test series (and optionally build number 'SERIES_NAME#BUILD_NUM' "
    ...
    group.add_argument("--changes", default=None, help=changes_help)
    group.add_argument(
        "--execution-id",
        default="default",
...
def main():
    ...
    for output_file in args.output_files:
        ...
        build_number_cache = parse_xml(
            output_file, args.format, connection, config, build_number_cache
        )
