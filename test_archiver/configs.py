...
def parse_key_value_pairs(values):
    ...
    for item in values or []:
        try:
            ...
            raise Exception(
                "Unsupported format for key-value pair: '{}' use NAME:VALUE".format(
                    item
                )
            )
    return pairs
...
class Config(metaclass=Singleton):
    ...
    def resolve(self, *, cli_args=None, file_config=None):
        ...
        file_config = file_config or (
            getattr(cli_args, "config_file", None) if cli_args else None
        )
    ...
    def _resolve_options(self):
        ...
        self.schema = self.resolve_option("schema")
        ...
        self.require_ssl = self.resolve_option(
            "require_ssl", default=False, cast_as=bool
        )
        ...
        # Schema updates
        self.allow_major_schema_updates = self.resolve_option(
            "allow_major_schema_updates", default=False, cast_as=bool
        )
        self.allow_minor_schema_updates = self.resolve_option(
            "allow_minor_schema_updates", default=False, cast_as=bool
        )
        # If major updates are allowed then minor ones are as well
        self.allow_minor_schema_updates = (
            self.allow_major_schema_updates or self.allow_minor_schema_updates
        )

        # Limit archived data
        self.archive_keywords = self.resolve_option(
            "archive_keywords", default=True, cast_as=bool
        )
        self.archive_keyword_statistics = self.resolve_option(
            "archive_keyword_statistics", default=True, cast_as=bool
        )
        self.ignore_logs = self.resolve_option(
            "ignore_logs", default=False, cast_as=bool
        )
        self.ignore_logs_below = self.resolve_option("ignore_logs_below", default=None)
        self.max_log_message_length = self.resolve_option(
            "max_log_message_length", cast_as=_log_message_length, default=2000
        )

        # Adjust timestamps
        self.time_adjust_secs = self.resolve_option(
            "time_adjust_secs", default=0, cast_as=int
        )
        self.time_adjust_with_system_timezone = self.resolve_option(
            "time_adjust_with_system_timezone", default=False, cast_as=bool
        )
    ...
    def resolve_option(self, name, default=None, cast_as=str):
        if (
            ...
            and self._cli_args.__getattribute__(name) is not None
        ):
...
def base_argument_parser(description):
    ...
    parser.add_argument(
        ...
        dest="config_file",
        help=(
            "Path to JSON config file containing database credentials and other "
    ...
    group.add_argument("--schema", help="database schema", default=None)
    ...
    group.add_argument(
        ...
        default=None,
        help=(
            "Allow TestArchiver to perform MINOR (backwards compatible) schema "
    ...
    group.add_argument(
        ...
        default=None,
        help=(
            "Allow TestArchiver to perform MAJOR (backwards incompatible) schema "
            "updates the test archive"
        ),
    )
    ...
    group.add_argument(
        "--no-keywords",
        dest="archive_keywords",
        action="store_false",
        default=None,
        help="Do not archive keyword data",
    )
    group.add_argument(
        "--no-keyword-stats",
        dest="archive_keyword_statistics",
        action="store_false",
        default=None,
        help="Do not archive keyword statistics",
    )
    group.add_argument(
        "--ignore-logs-below",
        default=None,
        choices=LOG_LEVEL_CUT_OFF_OPTIONS,
        help=(
            "Sets a cut off level for archived log messages. "
    ...
    group.add_argument(
        ...
        dest="time_adjust_with_system_timezone",
        default=None,
        action="store_true",
