...
class TestHelperFunctions(unittest.TestCase):
    def test_parse_key_value_pairs(self):
        ...
        self.assertEqual(
            configs.parse_key_value_pairs({"a": 1, "b": 2}), {"a": 1, "b": 2}
        )


FAKE_CONFIG_FILE_DATA = {
    "database": "archive.db",
    "user": "worker_user",
    "port": 1234,
...
class TestConfig(unittest.TestCase):
    def test_resolve_option(self):
        ...
        self.assertEqual(
            config.resolve_option("foo_option", default=10, cast_as=int), 100
        )
        ...
        self.assertEqual(
            config.resolve_option("foo_option", cast_as=configs._log_message_length), 0
        )
    ...
    def test_default_configs_are_resolved(self):
        ...
        self.assertEqual(config.require_ssl, False)
    ...
    def test_file_configs_are_resolved(self):
        ...
        self.assertEqual(
            config.metadata, {"version": "1.2.3", "environment": "integration"}
        )

    @mock.patch(
        "test_archiver.configs.read_config_file", return_value=FAKE_CONFIG_FILE_DATA
    )
    ...
    def test_cli_configs_are_resolved(self):
        fake_cli_args = argparse.Namespace(
            user="cli_user", port=1234, metadata=["foo:bar"]
        )
        config = configs.Config()
    ...
    def test_cli_configs_have_higher_precedence_than_config_files(self):
        fake_cli_args = argparse.Namespace(
            user="cli_user", port=4321, metadata=["version:3.2.1", "cli_data:foobar"]
        )
        config = configs.Config()
        ...
        self.assertEqual(
            config.metadata,
            {"version": "3.2.1", "cli_data": "foobar", "environment": "integration"},
        )
        #'metadata': {'version': '1.2.3', 'environment': 'integration'}
    ...
    def test_allowing_major_schema_update_overrides_minor_updates(self):
        ...
        fake_cli_args = argparse.Namespace(
            allow_major_schema_updates=True, allow_minor_schema_updates=False
        )
        config = configs.Config()
        config.resolve(cli_args=fake_cli_args)
        self.assertEqual(config.allow_minor_schema_updates, True)
...
class TestExecutionContext(unittest.TestCase):
    ...
    @mock.patch(
        "test_archiver.configs.read_config_file", return_value=FAKE_CHANGES_FILE_DATA_1
    )
    def test_execution_context_and_changes(self, fake_changes_file_data):
        fake_cli_args = argparse.Namespace(
            execution_context="PR", changes="foobar.json"
        )
        config = configs.Config()
        config.resolve(cli_args=fake_cli_args)
        self.assertEqual(config.execution_context, "PR")
    ...
    @mock.patch(
        "test_archiver.configs.read_config_file", return_value=FAKE_CHANGES_FILE_DATA_2
    )
